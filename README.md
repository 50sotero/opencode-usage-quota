...
{
  "$schema": "https://opencode.ai/tui.json",
  "plugin": [["/absolute/path/to/opencode-usage-quota/src/tui.tsx", { "refreshMs": 60000, "glyphs": "unicode" }]]
}
```

`glyphs` defaults to `unicode`; set it to `ascii` only if a terminal has broken glyph-width handling.
